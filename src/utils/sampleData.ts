...
export async function insertSampleData(): Promise<boolean> {
  try {
    ...
    const appointmentsPerPatient = Math.floor(
      totalAppointments / patientIds.length
    );

    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const sampleAppointments: Omit<
      Appointment,
    ...
    for (let i = 0; i < patientIds.length; i++) {
      for (let j = 0; j < appointmentsPerPatient; j++) {
        ...
        const daysOffset =
          Math.floor((j / (appointmentsPerPatient - 1)) * (daysRange * 2)) -
          daysRange;
        ...
        if (!date) {
          console.warn(
            `Nie udało się wygenerować unikalnego slotu dla pacjenta ${i}, wizyty ${j}`
          );
          continue;
        }
    ...
    for (let i = 0; i < additionalAppointments; i++) {
      ...
      const baseDate = new Date(
        today.getTime() + randomDay * 24 * 60 * 60 * 1000
      );
      ...
      if (!date) {
        console.warn(
          `Nie udało się wygenerować unikalnego slotu dla dodatkowej wizyty ${i}`
        );
        continue;
      }
      ...
      if (date < today) {
        status =
          Math.random() < 0.8
            ? AppointmentStatus.COMPLETED
            : statuses[2 + Math.floor(Math.random() * 2)];
      } else {
        status =
          Math.random() < 0.85
            ? AppointmentStatus.SCHEDULED
    ...
    console.log(
      `Dodano ${patientIds.length} pacjentów i ${sampleAppointments.length} wizyt`
    );
    console.log(
      `Średnio ${(sampleAppointments.length / daysRange).toFixed(1)} wizyt dziennie przez ${daysRange} dni`
    );
